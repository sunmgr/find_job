...
import { Calendar, Building2, CheckCircle2, Clock, ArrowRight, Layers } from 'lucide-react';
        ...
        <div className='mt-16'>
            ...
            <div className='flex items-end justify-between mb-8 px-2'>
                ...
                <div className='flex items-center gap-3 pb-1'>
                    <span className='text-xs font-bold text-slate-400 uppercase tracking-widest'>Total Applications</span>
            ...
            <div className='space-y-6'>
                {allAppliedJobs && allAppliedJobs.length > 0 ? allAppliedJobs.map((appliedJob) => (
                    <div 
                        key={appliedJob._id} 
                    ...
                    >
                            ...
                            <div className='flex flex-col gap-1.5'>
                                ...
                                <div className='flex items-center gap-6'>
                                    <div className='flex items-center gap-2'>
                                        ...
                                        <span className='text-xs font-bold text-slate-500 uppercase tracking-wider'>{appliedJob.job.company.name}</span>
                                    </div>
                                    <div className='flex items-center gap-2'>
                                        <Calendar className='w-3.5 h-3.5 text-slate-400' />
                                        <span className='text-xs font-bold text-slate-400'>Applied {appliedJob.job.createdAt.split("T")[0]}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className='flex items-center gap-6'>
                            <Badge 
                                className={`
                                    rounded-xl px-5 py-2 font-black text-[10px] uppercase tracking-[0.15em] border-none
                                    ${appliedJob.status === 'selected' 
                                        ? 'bg-emerald-50 text-emerald-700' 
                                        : 'bg-[#f5f1ee] text-[#4a3728]'}
                                `}
                            >
                                {appliedJob.status === 'selected' ? (
                                    <span className='flex items-center gap-2'><CheckCircle2 className='w-3 h-3'/> {appliedJob.status}</span>
                                ) : (
                                    <span className='flex items-center gap-2'><Clock className='w-3 h-3'/> {appliedJob.status}</span>
                                )}
                            </Badge>
                    ...
                    <div className='text-center py-16'>
                        <p className='text-slate-400 font-medium text-sm'>No applications yet. Start applying to jobs!</p>
                    </div>
                )}
            </div>
            ...
            <div className='mt-12 text-center'>
                <button className='group text-[11px] font-black uppercase tracking-[0.4em] text-slate-400 hover:text-[#4a3728] transition-all'>
                    Show Full History <span className='inline-block transition-transform group-hover:translate-x-1'>→</span>
                </button>
            </div>
        </div>
    );
